...
public class ShipInput : MonoBehaviour
{
    ...
    private float lastCameraUpdateTime = 0.1f;

    // How quickly the throttle reacts to input.
    private const float THROTTLE_SPEED = 0.0f;
    ...
    /// <summary>
    /// Use the C key to toggle between the two fighter camera views
    /// </summary>
    private void ToggleActiveCamera()
    {
        if (Input.GetKey(KeyCode.C))
        {
            ...
            trailCamera.enabled = !trailCamera.enabled;
            //lastCameraUpdateTime = Time.Now();
        }
    }
}